...
### 论坛规则、现实观鸟语义与角色初始化（2026-08-01）

生产部署 `15f4d0beb6a5d6a4cd7a1565802a5da76d0a5667`：

- 新增公开 `/rules`，把论坛总则、违规处理与六板块规则重构为语义化 HTML：桌面表格与双列板块卡片，390px 下违规处理改为纵向卡片；主导航与页脚均可达；
- 首页改为“真实的鸟类观察记录与东方主题创作”，观鸟板块改为“现实鸟类的观察记录”，图鉴改为“现实中的群鸟”；所有公开模板已清除“真实或虚构”“幻想乡鸟类”“现实与东方”等会混淆真实观鸟记录的旧表述；
- 图鉴、鸟类详情、用户观鸟计数和观鸟成就只统计「观鸟记录」板块；该板块新建、用户编辑和管理员编辑均在服务端强制鸟名与地点；
- 站务身份与贡献等级彻底分离：身份显示“站长/管理员”，贡献仍按帖子与回复积分计算，不再把站务身份列进贡献阶梯；
- 事务式初始化角色：用户 ID 2 `书书鸟Nipponia` 为唯一站长，用户 ID 3（生产实际用户名 `Alumin_Hyrdo`）为管理员；既有 ID 1 `marisa` 管理员按本次部署合同保留；三者均已验证且未禁言；
- 权限矩阵：站长能进入用户目录并看到 2 个管理员任免表单；两个管理员可看用户目录和管理帖子，但角色任免表单均为 0，页面明确“仅站长可以任命或撤销管理员”；匿名后台仍 302；
- 数据不变量保持 3 用户 / 6 板块 / 4 帖 / 3 回复 / 1 私信；`integrity_check=ok`、`foreign_key_check=[]`；Gunicorn 仍仅监听 `127.0.0.1:8001`；公网只开放 80/443，8000/8001 不可达；
- 本地自动测试 `47 passed`；独立最终审查 `PASS / blocker none`；公网 Chromium 在 390px 与 1280px 均无溢出，移动表格 324/324，三人身份/贡献徽章正确，console/page/HTTP/request error 均为 0；
